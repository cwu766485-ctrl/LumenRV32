...
if {$argc != 6} {
    error "usage: build_zu15eg_riscv_bram_uart.tcl <repo-root> <out-dir> <inst.data> <jobs> <cpu-clock-div> <rom-words>"
}
...
if {![string is integer -strict $rom_words] || $rom_words < 1} { error "rom-words must be a positive integer" }
file mkdir $out_dir
...
set_property verilog_define [list SOC_DDR_BOARD_MINIMAL \
    "FPGA_CPU_CLK_DIV=$cpu_clock_div" "RomNum=$rom_words" "FPGA_ROM_INIT=\"$inst_data_define\""] [get_filesets sources_1]
...
puts "ZU15EG_RISCV_BRAM_UART_ROM_WORDS=$rom_words"
