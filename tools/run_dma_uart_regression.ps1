param(
    ...
    [string]$Snapshot = "dma_uart_stream_tb",
    [int[]]$Seeds = @(5, 19, 43),
    [int]$Cases = 1
)
...
$buildDir = Join-Path $root "build\dma_uart_regression"
$rtlCoreDir = Join-Path $root "rtl\core"
$cfgFile = Join-Path $buildDir "tb_cfg.vh"
$sources = @(
    (Join-Path $root "tb\dma_uart_stream_tb.sv"),
...
try {
    ...
    foreach ($seed in $Seeds) {
        ...
        if (($output | Out-String) -notmatch "DMA_UART_STREAM_PASS") {
    ...
    Write-Host ("DMA UART regression PASS: {0}/{1} seeds" -f $passCount, $Seeds.Count)
    exit 0
}
finally {
    Pop-Location
