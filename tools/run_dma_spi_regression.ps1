param(
    ...
    [string]$Snapshot = "dma_spi_stream_tb",
    [int[]]$Seeds = @(7, 23, 61),
    [int]$Cases = 1
)
...
$buildDir = Join-Path $root "build\dma_spi_regression"
$rtlCoreDir = Join-Path $root "rtl\core"
$cfgFile = Join-Path $buildDir "tb_cfg.vh"
$sources = @(
    (Join-Path $root "tb\dma_spi_stream_tb.sv"),
...
try {
    ...
    foreach ($seed in $Seeds) {
        ...
        if (($output | Out-String) -notmatch "DMA_SPI_STREAM_PASS") {
    ...
    Write-Host ("DMA SPI regression PASS: {0}/{1} seeds" -f $passCount, $Seeds.Count)
    exit 0
}
finally {
    Pop-Location
