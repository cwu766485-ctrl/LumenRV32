param(
    ...
    [string]$Snapshot = "dma_mem2mem_tb",
    [int[]]$Seeds = @(3, 11, 29, 57),
    [int]$Cases = 6
)
...
$buildDir = Join-Path $root "build\dma_regression"
$rtlCoreDir = Join-Path $root "rtl\core"
$cfgFile = Join-Path $buildDir "tb_cfg.vh"
$sources = @(
    (Join-Path $root "tb\dma_mem2mem_tb.sv"),
...
try {
    ...
    foreach ($seed in $Seeds) {
        ...
        if (($output | Out-String) -notmatch "DMA_MEM2MEM_PASS") {
    ...
    Write-Host ("DMA regression PASS: {0}/{1} seeds" -f $passCount, $Seeds.Count)
    exit 0
}
finally {
    Pop-Location
